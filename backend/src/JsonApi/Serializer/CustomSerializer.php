...
namespace App\JsonApi\Serializer;
...
class CustomSerializer extends ArraySerializer
{
    ...
    public function includedData(ResourceInterface $resource, array $data)
    {
        ...
        foreach ($data as $value) {
            foreach ((array) $value as $includeObject) {
                if ($this->isNull($includeObject) || $this->isEmpty($includeObject)) {
                    continue;
                }

                // Custom Hack Because I needed to create inverse relationships from embedded fields for EmberJS
                if (
                  (
                      isset($includeObject['data']['type']) &&
                    !isset($includeObject['data']['attributes'])
                  )
                    &&
                  (
                      'profiles' == $includeObject['data']['type']
                  )
                ) {
                    continue;
                }
